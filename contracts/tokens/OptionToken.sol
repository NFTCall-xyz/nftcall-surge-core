...
contract OptionToken is IOptionToken, ERC721Enumerable, Ownable, SimpleInitializable {
    ...
    function openPosition(address to, OptionType optionType, uint256 strikeId, uint256 amount, uint256 maximumPremium) public override onlyVault returns(uint256)
    {
        if(amount == 0) {
            revert ZeroAmount(address(this));
        }
        uint256 positionId = _nextId++;
        _options[positionId] = OptionPosition(PositionState.PENDING, optionType, strikeId, amount, 0, maximumPremium);
        _totalValue += lockedValue(positionId);
        emit OpenPosition(to, positionId, optionType, strikeId, amount, maximumPremium);
